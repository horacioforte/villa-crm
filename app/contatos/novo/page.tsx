  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(payload?.message ?? "Falha ao cadastrar contato.");
      }
      ...
      if (error instanceof ZodError) {
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : "Nao foi possivel cadastrar o contato.",
        );
      }
    } finally {
  ...
  return (
    <main className="min-h-screen bg-[#F4F6FA] px-5 py-8 text-[#172033] sm:px-8">
      <div className="mx-auto max-w-4xl">
        <PageNavigation currentPage="Novo contato" currentHref="/contatos" />
        <Button
          variant="ghost"
