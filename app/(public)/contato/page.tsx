...
export default function ContatoPublicoPage() {
  ...
  return (
    <main className="min-h-screen bg-[#F4F6FA] text-[#1A2E5A]">
      <section className="mx-auto grid min-h-screen max-w-6xl gap-6 px-5 py-6 lg:grid-cols-[0.85fr_1.15fr] lg:items-center">
        <div className="rounded-[2rem] bg-[#1A2E5A] p-7 text-white shadow-xl shadow-[#1A2E5A]/20">
          <div className="inline-flex rounded-2xl bg-white p-2.5 text-[#1A2E5A]">
            <Truck className="size-6" />
          </div>
          <p className="mt-6 text-xs font-semibold uppercase tracking-[0.28em] text-white/60">
        ...
        <form
          ...
          className="rounded-[2rem] border border-[#D7DEEA] bg-white p-5 shadow-xl shadow-[#1A2E5A]/10 sm:p-6"
        >
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#1E4FAB]">
              Formulário de contato
            </p>
            <h2 className="mt-2 text-2xl font-bold">Envie sua mensagem</h2>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <Field label="Nome completo" required>
              <input
                ...
                className="h-11 w-full rounded-2xl border border-[#D7DEEA] bg-[#F4F6FA] px-4 outline-none focus:border-[#1E4FAB]"
          ...
          {status === "error" ? (
            <p className="mt-4 rounded-2xl bg-red-50 p-3 text-sm font-semibold text-red-700">
              Revise os campos obrigatórios ou tente novamente.
            </p>
          ) : null}
          ...
          <button
            ...
            className="mt-5 inline-flex h-11 w-full items-center justify-center gap-2 rounded-2xl bg-[#1E4FAB] px-5 font-semibold text-white transition hover:bg-[#1A2E5A] disabled:cursor-not-allowed disabled:opacity-60"
          >
            {isSubmitting ? (
              <>
...
function Field({
  ...
  return (
    <label className={`block ${className ?? ""}`}>
      <span className="mb-1.5 block text-sm font-semibold text-[#1A2E5A]">
        {label}
