  ...
  return (
    <Card className="h-full rounded-2xl border-[#D7DEEA] bg-white print:rounded-lg print:shadow-none">
      <CardContent className="flex h-full flex-col p-4 print:p-2.5">
        ...
        <div className="mt-2 flex gap-2 print:mt-1">
          <div className="flex-1 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 print:py-1.5">
            <p className="text-[9.5px] font-semibold uppercase text-emerald-700">Contratado</p>
            <p className="text-xl font-bold leading-tight text-emerald-800 print:text-base">{formatCurrency(ganhas.valor)}</p>
            <p className="text-[10px] text-emerald-700">
              {ganhas.quantidade} contrato{ganhas.quantidade === 1 ? "" : "s"}
            </p>
          </div>
          <div className="flex-1 rounded-xl border border-red-200 bg-red-50 px-3 py-2 print:py-1.5">
            <p className="text-[9.5px] font-semibold uppercase text-red-700">Perdido</p>
            <p className="text-xl font-bold leading-tight text-red-800 print:text-base">{formatCurrency(perdidas.valor)}</p>
        ...
        {evolucao.length > 0 ? (
          <div className="mt-2 flex-1 print:mt-1" style={{ minHeight: 60 }}>
          ...
          <p className="mt-2 flex-1 text-[11px] text-[#98A2B3] print:mt-1">Sem histórico suficiente para evolução.</p>
        )}

        <div className="mt-1 flex items-center gap-1.5 text-[11px] print:mt-0.5">
