...
export default function EmpresaDetalhePage() {
  ...
  useEffect(() => {
    async function loadEmpresa() {
      try {
        ...
        const data: EmpresaDetalhe = await response.json();
        setEmpresa(data);
        setForm({
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const savedEmpresa: EmpresaDetalhe = await response.json();
  ...
  return (
    <main className="min-h-screen bg-[#F4F6FA] px-5 py-8 text-[#172033] sm:px-8">
      <div className="mx-auto max-w-4xl">
        <PageNavigation
          currentPage={empresa?.nomeFantasia ?? empresa?.razaoSocial ?? "Empresa"}
          currentHref="/empresas"
        />
        <Button
          variant="ghost"
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <CardTitle className="mt-1 text-3xl font-bold text-[#1A2E5A]">
                  {isLoading ? "Carregando empresa..." : "Editar empresa"}
                </CardTitle>
              </div>
            </div>
          </CardHeader>
